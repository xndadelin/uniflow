...
export default function Error({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    // Let Next.js log this too; we just surface a friendly UI.
    console.error(error);
  }, [error]);
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col px-4 py-10">
      <section className="rounded-lg border border-destructive/40 bg-destructive/5 p-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">Nu am putut încărca pagina cu materiale. Poți încerca din nou.</p>
